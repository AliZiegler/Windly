...
import { Edit, Trash2, Phone, LampDesk, House, Heart, Plus, MapPin } from "lucide-react"
...
export default async function Page() {
    ...
    const [{ addressId: defaultAddressId }] = await db.select({ addressId: userTable.addressId }).from(userTable).where(eq(userTable.id, session.user.id))
    const defaultAddress = userAddresses.find((address) => address.id === defaultAddressId)
    ...
    const displayAddresses = userAddresses.map((address) => {
        const { addressType, name, phone, state, city, buildingNumber, id, street, zipCode } = address
        const isDefault = id === defaultAddressId

        return (
            <div key={id} className="relative bg-midnight border border-[#2a3038] rounded-lg p-4 sm:p-6 hover:shadow-lg transition-all duration-200 group">
                {isDefault && (
                ...
                <div className="flex items-center gap-3 mb-4 mt-2">
                    ...
                    <h3 className="font-bold text-lg text-white">
                        {capitalizeFirstLetter(addressType)}
                    </h3>
                </div>
                ...
                <div className="mb-4">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-300 text-sm font-medium">+964{phone}</span>
                    </div>
                </div>

                <div className="flex flex-col sm:flex-row gap-3">
                    <div className="flex gap-3 flex-1">
                        <Link href={`/account/address/${id}`} className="flex-1">
                            <button className="w-full bg-[#2a2f38] flex items-center justify-center gap-2 hover:bg-[#5a6578] text-white 
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                <div className="flex-1">
                    ...
                    {userAddresses.length === 0 ? (
                        <div className="flex flex-col items-center justify-center min-h-[300px] bg-midnight border-2 border-dashed border-[#4a5568] rounded-lg">
                ...
                <div className="hidden lg:block w-80 flex-shrink-0">
                    <div className="sticky top-6">
                        ...
                        <div className="bg-midnight border-2 border-[#00CAFF]/30 rounded-lg p-6 shadow-lg">
