...
export default async function Reviews({ userId }: { userId: string }) {
        ...
        return (
            <div className="flex flex-col w-full gap-6 p-4 lg:p-6 overflow-hidden">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    <div>
                        <h1 className="font-bold text-2xl lg:text-3xl text-white mb-2">{firstName}&apos;s Reviews</h1>
                        <p className="text-gray-400">
                            View all {name}&apos;s product reviews
                        </p>
                    </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center min-h-[400px] text-center bg-[#1e232b] rounded-xl border border-[#2a3038] p-6">
                    ...
                    <p className="text-gray-400 mb-4">
                        {name} haven&apos;t written any reviews yet.
                    </p>
                    <Link
                        href="/products"
    ...
    const displayReviews = reviews.map(({ id, createdAt, productName, productId, rating, reviewText }) => {
        ...
        <div className="flex flex-col w-full gap-6 p-4 lg:p-6 overflow-hidden">
            ...
            <div className="bg-[#1e232b] rounded-xl border border-[#2a3038] overflow-hidden">
                ...
                <div className="lg:hidden">
                    {reviews.map(({ id, createdAt, productName, productId, rating, reviewText }) => {
                        const date = new Date(createdAt);
                        const day = String(date.getDate()).padStart(2, "0");
                        const month = String(date.getMonth() + 1).padStart(2, "0");
                        const year = date.getFullYear();
