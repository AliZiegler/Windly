...
import { productTable, userTable, wishlistTable } from "@/db/schema";
...
export default async function UserWishlist({ id }: { id: string }) {
    ...
    const [{ name, gender }] = await db.select({ name: userTable.name, gender: userTable.gender }).from(userTable).where(eq(userTable.id, id));
        ...
        return (
            <div className="min-h-screen w-full p-4 sm:p-6 lg:p-8">
                <div className="max-w-7xl w-full">
                    <h1 className="text-2xl sm:text-3xl font-bold text-gray-100 mb-8">{firstName}&apos;s Wishlist</h1>
                    <div className="flex flex-col items-center justify-center w-full min-h-[60vh] text-center">
                        ...
                        <h2 className="text-xl sm:text-2xl font-semibold text-gray-200 mb-2">
                            {gender === "Male" ? "His" : gender === "Female" ? "Her" : `${firstName}'s`} wishlist is empty
                        </h2>
                    </div>
                </div>
            </div>
    ...
    const DisplayProducts = wishlistItems.map((item) => (
        <div
            key={item.productId}
            className="group relative bg-gradient-to-br from-midnight to-midnight/50 border border-gray-600/50 rounded-2xl 
            p-4 sm:p-6 transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-blue-500/10 hover:border-blue-400/30"
        >
