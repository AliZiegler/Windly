...
import { salePrice, urlString } from "@/app/components/global/Atoms";
...
export default async function CartPage() {
                    ...
                    <div className="text-center py-16">
                        <div className="w-24 h-24 mx-auto mb-6 rounded-full flex items-center justify-center" style={{ backgroundColor: "#2a313c" }}>
                            <svg className="w-12 h-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 3h2l.4 2M7 13h10l4-8H5.4m2.6 8L9 4H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17M17 13v4a2 2 0 01-2 2H9a2 2 0 01-2-2v-4m8 0V9a2 2 0 00-2-2H9a2 2 0 00-2 2v4.01" />
                            </svg>
                        </div>
                        <h2 className="text-2xl font-bold text-gray-200 mb-2">Your cart is empty</h2>
    ...
    const subtotal = cartItems.reduce((sum, item) => {
        const itemPrice = salePrice(item.productPrice, item.productDiscount);
        return sum + (parseFloat(itemPrice.replace('$', '')) * item.quantity);
    }, 0);
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-4">
                        {cartItems.map((item) => {
                            const itemPrice = salePrice(item.productPrice, item.productDiscount);
                            const itemTotal = parseFloat(itemPrice.replace('$', '')) * item.quantity;

                            return (
                                <div
                                    key={item.productId}
                                    className="border border-gray-600/50 rounded-2xl p-4 sm:p-6 transition-all duration-200 hover:border-gray-500/50"
                                            ...
                                            <div className="flex flex-col sm:flex-row justify-between gap-2">
                                                <div className="flex-1">
                                                    <Link
                                                        ...
                                                        className="text-lg font-semibold text-gray-100 hover:text-blue-400 transition-colors duration-200 line-clamp-2"
                                                    >
                                                        {item.productName}
                                                    </Link>
                                                    <div className="flex items-center gap-3 mt-2">
                                                        <span className="text-xl font-bold text-gray-100">{itemPrice}</span>
                                                    ...
                                                    <p className="text-sm text-gray-400 mt-1">
                                                        In Stock ({item.productStock} available)
                                                    </p>
                                                </div>
